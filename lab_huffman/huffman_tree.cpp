...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     for (auto curr = frequencies.begin(); curr != frequencies.end(); ++curr) {
     ...
     while (singleQueue.size() + mergeQueue.size() != 1) {
       ...
       for (int i = 0; i < 2; i++) {
         if (singleQueue.empty()) {
           tmp[i] = mergeQueue.front();
           mergeQueue.pop();
         } else if (mergeQueue.empty()) {
           tmp[i] = singleQueue.front();
           singleQueue.pop();
         } else {
            if (!(singleQueue.front()->freq < mergeQueue.front()->freq)) {
              tmp[i] = mergeQueue.front();
       ...
       TreeNode* iner = new TreeNode(tmp[0]->freq.getFrequency() + tmp[1]->freq.getFrequency());
       if (tmp[0]->freq < tmp[1]->freq) {
         iner->left = tmp[0];
         iner->right = tmp[1];
         mergeQueue.push(iner);
       } else {
         iner->left = tmp[1];
         iner->right = tmp[0];
         mergeQueue.push(iner);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if (bfile.getNextBit()) {
       Frequency fq(bfile.getNextByte(), 0);
       TreeNode* leaf = new TreeNode(fq);
       return leaf;
     } else {
