...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    auto curr = dict.find(Word1);
    auto other = dict.find(Word2);
    if (curr == dict.end() || other == dict.end()) {
      return false;
    }
    vector<string> homo = curr->second;
    vector<string> another = other->second;
    if (homo.size() != another.size()) {
      return false;
    }
    for (unsigned int i = 0; i < homo.size(); i++) {
      if (homo[i] != another[i]) {
        return false;
      }
    }
