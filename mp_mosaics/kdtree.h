...
class KDTree
{
  ...
  public:
    ...
    Point<Dim> quickSelect(vector<Point<Dim>>& newPoints, int a, int b, int k, int dim);

    int partition(vector<Point<Dim>>& newPoints, int a, int b, int pivot, int dim);

    void quickConstruct(vector<Point<Dim>>& newPoints, int a, int b, int dim, KDTreeNode*& curr);

    Point<Dim> findBest(const Point<Dim>& query, KDTreeNode* curr, int dim) const;

    void deleteTree(KDTreeNode* node);

    KDTreeNode* copyNode(KDTreeNode* other);
